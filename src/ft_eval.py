...
def main() -> None:
    ...
    if not adapter_path.exists() or not (adapter_path / "adapter_config.json").exists():
        raise FileNotFoundError(
            f"Adapter not found at: {adapter_path.resolve()}\n"
    ...
    print(f"Device: {device}  |  model: {model_name}  |  adapter: {adapter_path}")
    ...
    benchmark = read_jsonl(Path(args.benchmark))
    if args.limit:
        benchmark = benchmark[: args.limit]
    if not benchmark:
        raise ValueError(f"No items found in {args.benchmark}")
    ...
    for i, item in enumerate(benchmark, start=1):
        ...
        print(
            f"[{i}/{len(benchmark)}] overlap={overlap:.3f if overlap is not None else 'n/a'}"
